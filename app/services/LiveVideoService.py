...
def validate_if_user_in_area(user: UserSchema, event: EventSchema):
    ...
    coordinatesThatIntersectsWithUser = list(
        LiveVideoRepository.validate_if_user_in_area(userCoordinates, ObjectId(event.uid)))
    ...
    try:
        ...
        if not foundEventCursor._has_next():
            raise BadRequestException(
                f"No event found with the given eventId: {eventId}")
        event = foundEventCursor.next()
        eventOwnerFormattedStream = {}
        eventCreatedBy = ""
        # Gets the owner of the organizer club to validate if it is live streaming
        if (event['type'] == "Club"):
            placeOwnerOfEvent = get_by_id(ObjectId(event['clubId']))
            if placeOwnerOfEvent == None:
                raise BadRequestException(
                    f"The event was created by inexistent club: {event['clubId']}.")
            eventCreatedBy = placeOwnerOfEvent["ownerId"]
        else:
            eventCreatedBy = event['userId']
        eventOwnerStream = LiveVideoRepository.get_event_owner_stream(
            str(event['_id']), eventCreatedBy)
        ...
        raise InternalServerError(
            f'Validation error when creating Live stream object. Error: {str(e)}')
    except BadRequestException as e:
        raise BadRequestException(str(e))
    except Exception as e:
        raise InternalServerError(
            f'Error while retrieving live stream of owner: {eventCreatedBy} for event {event._id}. Error: {e}')


def create_live_stream(liveStreamData: CreateLiveSpaceDTO):
    try:
        createdLiveStream = LiveStreamVideoSchema(createdBy=liveStreamData.createdBy, eventId=liveStreamData.eventId,
                                                  channelId=liveStreamData.username, createdAt=str(datetime.now(ZoneInfo('UTC'))), photoUrl=liveStreamData.photoUrl)
