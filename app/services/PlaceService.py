...
def get_list_of_places(user: UserSchema, placeAttributesToFilter: PlaceSchema):
    ...
    formattedPlaces = format_places_list(
        user, placesList, placeAttributesToFilter)
...
def format_places_list(user: UserSchema, placesList, placeAttributesToFilter: PlaceSchema):
    ...
    for place in placesList:
        validatedDistanceBetweenPlaceAndUser = validate_if_place_inside_user_range(
            user, placeAttributesToFilter, place["location"]["mainCoordinates"]["coordinates"])
        if validatedDistanceBetweenPlaceAndUser >= 0:

            place_photos = build_photo_object_of_place(place)
...
def build_photo_object_of_place(place):
    ...
    for place_photo in place["placePhotos"]:
        place_photos.append(Photo(place_photo["filename"],
                                  place_photo["fileUrl"],
                                  place_photo["createdAt"]
                                  ).to_dict())
